import{d as B,o as D,h as V,c as _,f as r,i as l,j as L,b as k,F as U,k as O,a as I,l as F,_ as K}from"./index-Cm6XU2RW.js";import{u as H}from"./useVoice-B47mRVlh.js";const W=["src"],Y={class:"ipyc-design-page"},j={class:"visual-card"},G={class:"canvas-wrap"},X=70,q=45,z=50,J=50,Q=17.5,Z=21,x=60,tt=B({__name:"cartoon1",setup(at){const{voiceRef:E,audioSrc:M,showAudioStart:R,startVoiceAfterMounted:S,startVoiceByClick:h,stopVoiceBeforeRouteChange:P,handleVoiceEnded:b}=H("/yuehuabai/audio/5ipyc1.mp3");function N(){P(),F.push({name:"IPyCcartoon2"})}const i=O();let p=[],d=null,o=null;function u(c,t){const n=z+t*Math.cos(c),s=J+t*Math.sin(c);return{x:n,y:s}}function $(c){if(o?.sheet)try{o.sheet.insertRule(c,o.sheet.cssRules.length)}catch(t){console.warn("插入动画关键帧失败：",t)}}function w(){if(i.value)for(let c=0;c<X;c++){const t=Math.random()*2*Math.PI,n=Q+(Math.random()*2-1)*1.8,s=u(t,x),a=u(t,n),y=Math.random()*1,v=3+Math.random()*1,f=`black_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,m=`
    ...
    `;$(m);const e=document.createElement("div");e.className="particle-black",e.style.left=`${s.x}%`,e.style.top=`${s.y}%`,e.style.animation=`${f} ${v}s ease-out forwards`,e.style.animationDelay=`${y}s`,i.value.appendChild(e),p.push(e)}}function A(){if(i.value)for(let c=0;c<q;c++){const t=Math.random()*2*Math.PI,n=Z+(Math.random()*2-1)*1.5,s=u(t,x),a=u(t,n),y=2.5+Math.random()*1.2,v=2.8+Math.random()*1,f=`white_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,m=`
      @keyframes ${f} {
        0% {
          left: ${s.x}%;
          top: ${s.y}%;
